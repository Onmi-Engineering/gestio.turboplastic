...
class SaleOrder(models.Model):
    ...
    carriage_ids = fields.Many2many('carriage.order', compute='_compute_carriage_ids', string='Carriages associated to this manufacturing order')
    ...
    def _get_action_view_carriage(self, carriages):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id(
            "onmi_turbo_weighings.onmi_action_carriage_order")

        if len(carriages) > 1:
            action['domain'] = [('id', 'in', carriages.ids)]
            action['view_id'] = (
                self.env.ref('onmi_turbo_weighings.carriage_order_tree_view_for_action').id,
            ...
            form_view = [
                (self.env.ref('onmi_turbo_weighings.carriage_order_form_view_for_action').id, 'form')]
            if 'views' in action:
    ...
    def action_create_carriage_order(self):
        ...
        return {
            ...
            'views': [
                (self.env.ref('onmi_turbo_weighings.carriage_order_form_view_for_action').id, 'form')],
            'view_id': self.env.ref('onmi_turbo_weighings.carriage_order_form_view_for_action').id,
            'target': 'new',
            'context': ctx,
        }
